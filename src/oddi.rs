...
fn main() -> io::Result<()> {
    ...
    for i in 0..num_threads {
        ...
        if i == 0 {
            ...
            sec = (i.to_biguint().unwrap() * 1_000_000.to_biguint().unwrap()) + 1.to_biguint().unwrap();
        }
        let end: BigUint = (i + 1).to_biguint().unwrap() * 1_000_000.to_biguint().unwrap();
        children.push(thread::spawn(move || {
            ...
            loop {
                ...
                if num.rem_euclid(&2.to_biguint().unwrap()) != 0.to_biguint().unwrap() {
